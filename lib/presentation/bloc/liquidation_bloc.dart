...
class LiquidationBloc extends ChangeNotifier {
  ...
  void init(
      ...
      required DateTime liqDate}) async {
    _userID = userID;
    _businessID = businessID;
    _liqDate = liqDate;
    _isLoading = true;
    final responseAmount = await APIServices.getLiquidationAmount(
        userID: userID,
        liqDate: Config.formatDate(liqDate, 'yyyy-MM-dd'),
        businessID: businessID);
    _responseModel = responseAmount;
    if (responseAmount!.statusCode == 200) {
      _liquidationAmount = responseAmount.data;
      final responseDetails = await APIServices.getLiquidationDetails(
          userID: userID,
          liqDate: Config.formatDate(liqDate, 'yyyy-MM-dd'),
          businessID: businessID);
  ...
  Future<String> getReportExcel() async {
    _isLoading = true;
    notifyListeners();
    final responseExcel = await APIServices.getReportLiquidationExcel(
        userID: _userID,
        liqDate: Config.formatDate(_liqDate!, 'yyyy-MM-dd'),
  ...
  Future<String> setFile64xls(String s) async {
    ...
    File file = File(
        '${dir.path}/REPORTVENTA-${now.year}${now.month}${now.day}${now.hour}${now.minute}${now.second}.xlsx');
